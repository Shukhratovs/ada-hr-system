          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50/70 border-b border-slate-100">
                ...
                <th className="px-4 py-3.5 text-center text-[11px] font-semibold text-slate-400 uppercase tracking-wider">PIN</th>
              ...
              {filtered.map((emp) => (
                <tr key={emp.id} className={`hover:bg-slate-50/60 transition-colors duration-100 ${!emp.active ? 'opacity-50' : ''}`}>
                  ...
                  <td className="px-4 py-3.5 text-center">
                    {emp.pin ? (
                      <span className="inline-flex items-center gap-1 bg-emerald-50 text-emerald-600 text-[11px] font-bold px-2 py-0.5 rounded-full">
                      ...
                      <span className="inline-flex items-center gap-1 bg-slate-100 text-slate-400 text-[11px] font-medium px-2 py-0.5 rounded-full">
                        {lang === 'uz' ? 'Yo\'q' : 'Нет'}
                      </span>
                    )}
                  </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={() => setDeleteConfirm(null)}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm mx-4 p-6" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex gap-3">
              <button onClick={() => setDeleteConfirm(null)} className="flex-1 py-2.5 rounded-xl border border-slate-200 text-sm font-semibold text-slate-600 hover:bg-slate-50 transition-colors">
                {lang === 'uz' ? 'Bekor qilish' : 'Отмена'}
              </button>
              <button onClick={deleteEmployee} disabled={deleting} className="flex-1 py-2.5 rounded-xl bg-red-600 hover:bg-red-700 text-white text-sm font-semibold transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
